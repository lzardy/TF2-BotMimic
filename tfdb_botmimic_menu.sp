/**
 * TFDB Bot Mimic Menu - Record flicks and have TFDB bots replay them!
 * Admin menu integration and menu interface.
 * by Darth (and Peace-Maker)
 * visit https://gitlab.com/darth_lane
 * 
 * Changelog:
 * 1.0   - 28.05.2019: Released rewrite of Bot Mimic Menu for TF2 and the gamemode TFDB.
 */
...
{
	name = "TFDB Bot Mimic Menu",
	author = "Darth and Jannik \"Peace-Maker\" Hartung",
	description = "Handle records and record own flicks for TFDB.",
...
{
	ServerCommand("sv_cheats 1;bot_kick all;sv_cheats 0");
	RegConsoleCmd("sm_mimic", Cmd_Record, "Opens the bot mimic menu");
	RegConsoleCmd("sm_stoprecord", Cmd_StopRecord, "Stops your current record");
	RegConsoleCmd("sm_savebookmark", Cmd_SaveBookmark, "Saves a bookmark with the given name in the record the target records. sm_savebookmark <name|steamid|#userid> <bookmark name>");
	//RegAdminCmd("sm_mimic", Cmd_Record, ADMFLAG_CONFIG, "Opens the bot mimic menu", "tfdb_botmimic");
	//RegAdminCmd("sm_stoprecord", Cmd_StopRecord, ADMFLAG_CONFIG, "Stops your current record", "tfdb_botmimic");
	//RegAdminCmd("sm_savebookmark", Cmd_SaveBookmark, ADMFLAG_CONFIG, "Saves a bookmark with the given name in the record the target records. sm_savebookmark <name|steamid|#userid> <bookmark name>", "tfdb_botmimic");
...
public void OnClientPutInServer(int client)
{
	...
	{
		TFDB_BotMimic_PlayRecordFromFile(client, g_sSupposedToMimic[client]);
	}
}
...
public Action Cmd_Record(int client, int args)
{
	...
	if(TFDB_BotMimic_IsPlayerRecording(client))
	{
...
public Action Cmd_SaveBookmark(int client, int args)
{
	...
	if(!TFDB_BotMimic_IsPlayerRecording(iTarget))
	{
	...
	TFDB_BotMimic_SaveBookmark(iTarget, sBookmarkName);
...
public Action CmdLstnr_Say(int client, const char[] command, int argc)
{
	...
	if(g_bRenameRecord[client])
	{
		...
		BMError error = TFDB_BotMimic_ChangeRecordName(g_sPlayerSelectedRecord[client], sText);
		if(error != BM_NoError)
		{
			char sError[64];
			TFDB_BotMimic_GetErrorString(error, sError, sizeof(sError));
	...
	else if(g_bEnterCategoryName[client])
	{
		...
		ArrayList hCategoryList = TFDB_BotMimic_GetLoadedRecordCategoryList();
 ...
 * Bot Mimic Callbacks
 */
public void TFDB_BotMimic_OnRecordSaved(int client, char[] name, char[] category, char[] subdir, char[] file)
{
...
public void TFDB_BotMimic_OnRecordDeleted(char[] name, char[] category, char[] path)
{
...
public Action TFDB_BotMimic_OnStopRecording(int client, char[] name, char[] category, char[] subdir, char[] path, bool &save)
{
...
public int Menu_SelectCategory(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			if(TFDB_BotMimic_IsPlayerRecording(param1))
			{
			...
			if(!IsPlayerAlive(param1) || GetClientTeam(param1) < view_as<int>(TFTeam_Red))
			{
			...
			if(TFDB_BotMimic_IsPlayerMimicing(param1))
			{
			...
			TFDB_BotMimic_StartRecording(param1, sTempName, DEFAULT_CATEGORY);
			DisplayRecordInProgressMenu(param1);
		}
...
void DisplayRecordMenu(int client)
{
	...
	ArrayList hRecordList = TFDB_BotMimic_GetLoadedRecordList();
	...
	for(int i=0;i<iSize;i++)
	{
		...
		TFDB_BotMimic_GetFileCategory(sPath, sCategory, sizeof(sCategory));
		if(!StrEqual(g_sPlayerSelectedCategory[client], sCategory))
			continue;
		
		TFDB_BotMimic_GetFileHeaders(sPath, iFileHeader);
		...
		{
			if(IsClientInGame(c) && TFDB_BotMimic_IsPlayerMimicing(c))
			{
				TFDB_BotMimic_GetRecordPlayerMimics(c, sPlayerPath, sizeof(sPlayerPath));
...
public int Menu_SelectRecord(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(StrEqual(info, "record"))
		{
			...
			Format(sTempName, sizeof(sTempName), "%d_%d", GetTime(), param1);
			g_bPlayerRecordingFromMenu[param1] = true;
			TFDB_BotMimic_StartRecording(param1, sTempName, g_sPlayerSelectedCategory[param1]);
			DisplayRecordInProgressMenu(param1);
		}
		else
		{
...
void DisplayRecordDetailMenu(int client)
{
	...
	if(TFDB_BotMimic_GetFileHeaders(g_sPlayerSelectedRecord[client], iFileHeader) != BM_NoError)
	{
		g_sPlayerSelectedRecord[client][0] = '\0';
		DisplayRecordMenu(client);
		return;
...
public int Menu_HandleRecordDetails(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		int iFileHeader[BMFileHeader];
		if(TFDB_BotMimic_GetFileHeaders(g_sPlayerSelectedRecord[param1], iFileHeader) != BM_NoError)
		{
			g_sPlayerSelectedRecord[param1][0] = '\0';
			DisplayRecordMenu(param1);
			return;
		...
		if(StrEqual(info, "playselect"))
		{
			...
			for(int i=1;i<=MaxClients;i++)
			{
				if(IsClientInGame(i) && IsFakeClient(i) && GetClientTeam(i) >= view_as<int>(TFTeam_Red) && !IsClientSourceTV(i) && !IsClientReplay(i))
				{
					IntToString(GetClientUserId(i), sUserId, sizeof(sUserId));
					Format(sBuffer, sizeof(sBuffer), "%N", i);
					
					if(GetClientTeam(i) == view_as<int>(TFTeam_Red))
						Format(sBuffer, sizeof(sBuffer), "%s [RED]", sBuffer);
					else
						Format(sBuffer, sizeof(sBuffer), "%s [BLU]", sBuffer);
					
					if(TFDB_BotMimic_IsPlayerMimicing(i))
					{
						TFDB_BotMimic_GetRecordPlayerMimics(i, sPath, sizeof(sPath));
						TFDB_BotMimic_GetFileHeaders(sPath, iFileHeader);
		...
		else if(StrEqual(info, "playadd"))
		{
			...
			hMenu.AddItem("blu", "Blu");
			hMenu.AddItem("red", "Red");
		...
		else if(StrEqual(info, "stop"))
		{
			...
			for(int i=1;i<=MaxClients;i++)
			{
				if(IsClientInGame(i) && TFDB_BotMimic_IsPlayerMimicing(i))
				{
					TFDB_BotMimic_GetRecordPlayerMimics(i, sPath, sizeof(sPath));
					if(StrEqual(sPath, g_sPlayerSelectedRecord[param1]))
					{
						...
						GetClientName(i, name, PLATFORM_MAX_PATH);
		...
		{
			int iCount = TFDB_BotMimic_DeleteRecord(g_sPlayerSelectedRecord[param1]);
...
public int Menu_SelectBotToMimic(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(!iBot || !IsClientInGame(iBot) || GetClientTeam(iBot) < view_as<int>(TFTeam_Red))
		{
		...
		char sPath[PLATFORM_MAX_PATH];
		if(TFDB_BotMimic_IsPlayerMimicing(iBot))
		{
			TFDB_BotMimic_GetRecordPlayerMimics(iBot, sPath, sizeof(sPath));
			// That bot already plays this record. stop that.
			if(StrEqual(sPath, g_sPlayerSelectedRecord[param1]))
			{
				TFDB_BotMimic_StopPlayerMimic(iBot);
			...
			{
				TFDB_BotMimic_StopPlayerMimic(iBot);
				TFDB_BotMimic_PlayRecordFromFile(iBot, g_sPlayerSelectedRecord[param1]);
...
public int Menu_SelectBotTeam(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(StrEqual(info, "blu"))
		{
			ServerCommand("sv_cheats 1");
			ServerCommand("bot -team blu");
			ServerCommand("sv_cheats 0");
		}
		else
		{
			ServerCommand("sv_cheats 1");
			ServerCommand("bot -team red");
			ServerCommand("sv_cheats 0");
			PrintToServer("Spawning red bot.");
		}
...
void DisplayBookmarkListMenu(int client)
{
	...
	if(TFDB_BotMimic_GetRecordBookmarks(g_sPlayerSelectedRecord[client], hBookmarks) != BM_NoError)
	{
		g_sPlayerSelectedRecord[client][0] = '\0';
		DisplayRecordMenu(client);
		return;
...
void DisplayBookmarkMimicingPlayers(int client)
{
	...
	for(int i=1;i<=MaxClients;i++)
	{
		...
		TFDB_BotMimic_GetRecordPlayerMimics(i, sBuffer, sizeof(sBuffer));
...
public int Menu_HandleBookmarkMimicingPlayer(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(!iTarget || !IsClientInGame(iTarget) || GetClientTeam(iTarget) < view_as<int>(TFTeam_Red))
		{
			PrintToChat(param1, "[BotMimic] The bot you selected can't be found anymore.");
			DisplayBookmarkMimicingPlayers(param1);
			return;
		...
		if(!TFDB_BotMimic_IsPlayerMimicing(iTarget))
		{
		...
		else
		{
			...
			TFDB_BotMimic_GetRecordPlayerMimics(iTarget, sRecordPath, sizeof(sRecordPath));
		...
		TFDB_BotMimic_GoToBookmark(iTarget, g_sPlayerSelectedBookmark[param1]);
		DisplayBookmarkMimicingPlayers(param1);
	}
	else if (action == MenuAction_Cancel)
	{
...
void DisplayRecordInProgressMenu(int client)
{
	...
	if(TFDB_BotMimic_IsRecordingPaused(client))
...
public int Menu_HandleRecordProgress(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			if(!TFDB_BotMimic_IsRecordingPaused(param1))
			{
				TFDB_BotMimic_PauseRecording(param1);
		...
		else if(StrEqual(info, "resume"))
		{
			if(TFDB_BotMimic_IsRecordingPaused(param1))
			{
				TFDB_BotMimic_ResumeRecording(param1);
		...
		else if(StrEqual(info, "save"))
		{
			...
			TFDB_BotMimic_StopRecording(param1, true);
		}
		else if(StrEqual(info, "discard"))
		{
			TFDB_BotMimic_StopRecording(param1, false);
			DisplayRecordMenu(param1);
		}
	}
	else if (action == MenuAction_Cancel)
...
public void TopMenu_NewRecord(TopMenu topmenu, TopMenuAction action, TopMenuObject topobj_id, int param, char[] buffer, int maxlength)
{
	...
	{
		if(!IsPlayerAlive(param) || GetClientTeam(param) < view_as<int>(TFTeam_Red))
		{
		...
		if(TFDB_BotMimic_IsPlayerRecording(param))
		{
		...
		if(TFDB_BotMimic_IsPlayerMimicing(param))
		{
		...
		TFDB_BotMimic_StartRecording(param, sTempName, DEFAULT_CATEGORY);
